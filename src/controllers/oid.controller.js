...
const Repository = require('../repositories/oid.repository');
...
exports.getById = async (req, res, next) => {
  try {
    ...
    if (!data) return res.status(400).json({ error: 'OID não localizado.' })
...
exports.post = async (req, res, next) => {
  ...
  try {
    ...
    res.status(500).json({
      message: 'Falha ao cadastrar OID, ' + e.message
    });
  }
};
...
exports.put = async (req, res, next) => {
  try {
    ...
    res.status(500).json({
      message: 'Falha ao atualizar OID, ' + e
    });
  }
};
...
exports.delete = async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      message: 'OID removido com sucesso!'
    });
  } catch (e) {
    res.status(500).json({
      message: 'Falha ao remover OID'
    });
  }
};
